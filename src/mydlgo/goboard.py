...
class GoString:
    def __init__(self, color: Player, stones: StringOrLiberty, liberties: StringOrLiberty):
        self.color = color
        self.stones = frozenset(stones)
        self.liberties = frozenset(liberties)

    def without_liberty(self, point: Point) -> GoString:
        new_liberties = self.liberties - frozenset([point])
...
class GameState:
    def __init__(self, board: Board, next_player: Player, previous: Optional[GameState], move: Optional[Move]):
        ...
        self.previous_states: FrozenSet = (
            frozenset(previous.previous_states | {(previous.next_player, previous.board.zobrist_hash())})
